...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const articleBody =
    document.querySelector('.blog-post-content .rte-formatter') ||
  ...
  /**
   * Safely get the value of a dataset key
   * @param {string} key
   * @returns {string}
   */
  ...
  html = html.replace(/\[image(\d*)\]/gi, (_, n) => {
    ...
    if (!url) return '';
    return `<figure class="bideshi-inline-image"><img src="${url}" alt="" class="article-image" loading="lazy"/></figure>`;
  });

  html = html.replace(/\[audio(\d*)\]/gi, (_, n) => {
    const url = getMedia(`audio${n || 0}`);
    if (!url) return '';
    return `<div class="bideshi-inline-audio"><audio controls preload="none" class="article-audio"><source src="${url}" type="audio/mpeg">Your browser does not support the audio element.</audio></div>`;
  });

  html = html.replace(/\[video(\d*)\]/gi, (_, n) => {
    const url = getMedia(`video${n || 0}`);
    if (!url) return '';
    return `<video controls preload="metadata" class="article-video"><source src="${url}" type="video/mp4">Your browser does not support the video tag.</video>`;
  });
